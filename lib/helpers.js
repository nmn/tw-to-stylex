...
exports.getConvertedClasses = exports.convertFromCssToJss = void 0;
...
const convertToCss = classNames => {
  let cssCode = '';
  _cheatsheet.default.forEach(element => {
...
const getBreakPoints = (input, breakpoint) => {
  return input.replaceAll('\n', ' ').split(' ').filter(i => i.startsWith(breakpoint + ':')).map(i => i.substring(3));
};
const getHoverClass = input => {
  return input.replaceAll('\n', ' ').split(' ').filter(i => i.startsWith('hover:')).map(i => i.replace('hover:', ''));
};
const getConvertedClasses = input => {
  if (input === '') return '';
  const classNames = input.split(/\s+/).map(i => i.trim()).filter(i => i !== '');
  const breakpoints = _cheatsheet.default[0].content[1].table;
...
exports.getConvertedClasses = getConvertedClasses;
const convertFromCssToJss = css => {
  try {
    const root = _postcss.default.parse(css);
    return _postcssJs.default.objectify(root);
  } catch (e) {
    console.log(e);
    return null;
  }
};
exports.convertFromCssToJss = convertFromCssToJss;